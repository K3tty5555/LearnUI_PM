...
![首页：78 个可交互 UI 标本](docs/assets/home-hero.png)
...
**当前构建：** 258 个静态页面；78 个 UI 标本、45 个风格标本、12 个页面参考、74 个知名网站设计规范；数据校验、筛选、选择导出、状态切换和移动端布局已通过回归测试；`python3 build.py` 一条命令重建全站。
...
- **78 个词条**：47 个 Web + 31 个 macOS，目录直接播放原始 HTML/CSS/JS 动效，详情页保留交互；离屏或切到后台时演示停止运行。
- **78 个详情页**：解剖（每个部件的名字）、Prompt、调试 Prompt、代码符号表、相关词条
...
- **74 个知名网站设计规范**：来自 `VoltAgent/awesome-design-md`，可按品牌、行业和设计气质筛选，按滚动自动分批展示并提供完整 DESIGN.md
...
python3 build.py          # 生成 site/（258 个静态页面）
...
│   └── ui.json         # 站点文案（双语）
├── demos/<slug>.html   # 78 个 UI 标本 + 44 个 style-<slug>.html 风格标本
...
- **78 entries** (47 Web + 31 macOS), each with a **live interactive specimen** (real HTML/CSS/JS, not screenshots)
...
- **78 detail pages**: anatomy of every part, copy-ready agent prompt, debug prompt, API symbol table, related entries
...
动态标本回归：`python3 scripts/test-live-demos.py site --base /LearnUI_PM`（需 Playwright/Chromium，支持 `PLAYWRIGHT_CHROME_PATH` 指定本地浏览器）。CI 实测动效变化、全部 78 个标本、移动端、语言切换、减少动态效果、离屏停止与失败重试。
