...
UI 元素词典保留全部 78 个真实动态标本：进入视口附近 100px 时，在隔离演示帧内运行原始 HTML/CSS/JS；离屏、筛选隐藏和页面后台时移除演示帧，停止动画、定时器和事件监听。前三个标本代码内嵌；其余从 api/specimens/ 按需读取并在本页复用。详情交互不变，无 JavaScript 时保留图片回退。其它参考目录继续使用轻量双语预览。
...
- **目录**：UI 词典显示自动播放的控件标本、名称、简短说明与加入参考按钮。标本区域 inert，整卡打开详情；隔离帧只允许执行内置演示脚本。动效加载失败时明确反馈，网络恢复后可重试。其它目录保留图片预览。
...
- UI 词典必须能直接看到动效。性能控制约束同时运行的演示数量与离屏生命周期，不能删掉动态标本。隔离演示使用系统 UI 字体，避免每个演示额外请求字体；外壳保持 Geist。
- 不预下载整站，不用无限缓存掩盖网络问题。
