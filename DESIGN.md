...
UI 元素词典保留全部 62 个真实动态标本：进入视口附近 100px 时，在隔离演示帧内运行原始 HTML/CSS/JS；离屏、筛选隐藏和页面后台时移除演示帧，停止动画、定时器和事件监听。前三个标本代码内嵌；其余从 api/specimens/ 按需读取并在本页复用。详情交互不变，无 JavaScript 时保留图片回退。其它参考目录继续使用轻量双语预览。
